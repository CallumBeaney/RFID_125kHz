...
CTS = 1 # Jumper 1, also known as GPIO18
GPIO_PIN = 5
...
def WaitForCTS():
    ...
    while wiringpi2.digitalRead(CTS):
        # do nothing
        time.sleep(0.001)
    return
...
def ReadInt(fd):
    ...
    qtydata = wiringpi2.serialDataAvail(fd)   
    # print ("ReadInt function read %s bytes of data" % qtydata)    # Added for debug purposes
...
def RFIDSetup():
    ...
    fd = wiringpi2.serialOpen('/dev/serial1', 9600)
    ...
    if response == 0 and fd >0:
        ...
        print("WiringPi setup status is '" + str(response) + "' (expected: 0).\nSerial port is " + str(fd) + " (expected a value greater than 0).\nExiting program.")
        sys.exit()
...
def SetPollingDelay(fd):
    ...
    if ans == int("0xC0", 16):
        ...
        print ("Unexpected response %s (expected: C0)" % hex(ans))
        # flush any remaining characters from the buffer
        wiringpi2.serialFlush(fd)
    return

def ReadTagPage(fd, page=None):
...
def HelpText():
    ...
    print ("R - Read Tag and PAGE of data")
    print ("r - Read Tag and BLOCK of data")
